  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Configuring D4nc3rHardwareInterface...");
  ...
  if (info_.hardware_parameters.count("encoder_topic_name"))
  {
    ...
    RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Using encoder topic: %s", encoder_topic_name_.c_str());
  }
  if (info_.hardware_parameters.count("ticks_per_wheel_revolution"))
  {
    ticks_per_wheel_revolution_ = std::stod(info_.hardware_parameters["ticks_per_wheel_revolution"]);
     RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Ticks per wheel revolution: %.2f", ticks_per_wheel_revolution_);
  }
   if (info_.hardware_parameters.count("left_encoder_data_index"))
  {
    ...
    RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Using left encoder data index: %d", left_encoder_data_index_);
  }
  if (info_.hardware_parameters.count("right_encoder_data_index"))
  {
    ...
    RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Using right encoder data index: %d", right_encoder_data_index_);
  }
  ...
  {
    RCLCPP_ERROR(
      rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"),
  ...
  // This assumes your diff_drive_controller uses 'left_wheel_joint' and 'right_wheel_joint'
  // And these are defined in your URDF's ros2_control tag.
  for (size_t i = 0; i < info_.joints.size(); ++i) {
      if (info_.joints[i].name == "left_wheel_joint") { // This name should match your URDF and controller config
          left_wheel_joint_name_ = info_.joints[i].name;
          left_wheel_joint_index_ = i;
          RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Found left wheel joint: %s at index %d", left_wheel_joint_name_.c_str(), left_wheel_joint_index_);
      } else if (info_.joints[i].name == "right_wheel_joint") { // This name should match your URDF and controller config
          right_wheel_joint_name_ = info_.joints[i].name;
          right_wheel_joint_index_ = i;
          RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Found right wheel joint: %s at index %d", right_wheel_joint_name_.c_str(), right_wheel_joint_index_);
      }
  }

  if (left_wheel_joint_index_ == -1 || right_wheel_joint_index_ == -1) {
      RCLCPP_ERROR(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Could not find 'left_wheel_joint' or 'right_wheel_joint'. Check URDF and controller config.");
      return hardware_interface::CallbackReturn::ERROR;
  }
  ...
  for (const auto& joint : info_.joints) {
    for (const auto& si : joint.state_interfaces) {
      if (si.name != hardware_interface::HW_IF_POSITION && si.name != hardware_interface::HW_IF_VELOCITY) {
        RCLCPP_ERROR(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Joint '%s' has unexpected state interface '%s'", joint.name.c_str(), si.name.c_str());
        return hardware_interface::CallbackReturn::ERROR;
      }
    }
  }

  RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Configuration successful.");
...
{
  RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Configuring subscriber node...");
  ...
  if (!subscriber_node_) {
      RCLCPP_ERROR(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Failed to create subscriber node.");
      return hardware_interface::CallbackReturn::ERROR;
  }
  ...
  if (!encoder_subscriber_) {
      RCLCPP_ERROR(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Failed to create encoder subscriber for topic: %s", encoder_topic_name_.c_str());
      return hardware_interface::CallbackReturn::ERROR;
  }
  ...
  RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Subscriber configuration successful.");
  return hardware_interface::CallbackReturn::SUCCESS;
}
...
std::vector<hardware_interface::StateInterface> D4nc3rHardwareInterface::export_state_interfaces()
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Exported state interfaces.");
...
std::vector<hardware_interface::CommandInterface> D4nc3rHardwareInterface::export_command_interfaces()
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Exported command interfaces (velocity).");
...
hardware_interface::CallbackReturn D4nc3rHardwareInterface::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Activating D4nc3rHardwareInterface...");
  ...
  if (subscriber_node_ && !executor_) { // Ensure node exists and executor not already started
    ...
    executor_thread_ = std::thread([this]() { 
        RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Starting executor spin for subscriber node.");
        this->executor_->spin(); 
        RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Executor spin finished for subscriber node.");
    });
  }
  ...
  RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Activation successful.");
  return hardware_interface::CallbackReturn::SUCCESS;
}
...
hardware_interface::CallbackReturn D4nc3rHardwareInterface::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Deactivating D4nc3rHardwareInterface...");
  if (executor_) {
    executor_->cancel();
  }
  ...
  RCLCPP_INFO(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Deactivation successful.");
  return hardware_interface::CallbackReturn::SUCCESS;
}
...
void D4nc3rHardwareInterface::encoder_callback(const std_msgs::msg::Int32MultiArray::SharedPtr msg)
{
  ...
  if (msg->layout.dim.empty() || msg->layout.dim[0].size < 2) {
    RCLCPP_WARN_THROTTLE(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), *subscriber_node_->get_clock(), 1000, 
  ...
  // RCLCPP_DEBUG(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), "Received encoder ticks: L=%d, R=%d", 
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  if (ticks_per_wheel_revolution_ <= 0) {
      RCLCPP_ERROR_THROTTLE(rclcpp::get_logger("d4nc3r1::D4nc3rHardwareInterface"), *subscriber_node_->get_clock(), 1000,
                           "ticks_per_wheel_revolution is not positive. Cannot calculate position.");
